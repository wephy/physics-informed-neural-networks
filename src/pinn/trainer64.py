"""Double-precision training loop.

Time-dependent problems are solved slab by slab: each slab trains a fresh
network against the previous slab's solution as its initial condition, in
two passes — a cheap float32 pass for early progress, then a float64 pass
to converge.  Steady problems are a single slab.  trainer32 is the
single-precision counterpart, identical apart from the precision passes.
"""
...
def compute_l2_error(
    ...
    for i, ch in enumerate(channels):
        ...
        rel_err  = abs_err / (ref_norm + 1e-17)
...
def precompile(problem_class, config):
    ...
    Runs two solver steps in both float32 and float64 so that a subsequent
    `train()` call with the same shapes and settings skips compilation entirely.
    Safe to call multiple times; a cache hit is silent and instant.

    The cache is written to `config.cache_dir` (default `.cache/jax`).
    """
    config.resolve_precision_defaults(double_precision=True)
    ...
    # Mirror train()'s dry run exactly so the cached graphs match: opt_state is
    # initialised once (float32) and carried unchanged into the float64 pass,
    # giving the same mixed-dtype step that train() compiles and reuses.
    weights   = None
    opt_state = None
    for precision_pass, precision_bits in enumerate([32, 64]):
        jax.config.update("jax_enable_x64", precision_bits == 64)

        _pass_dtype = jnp.float64 if precision_bits == 64 else jnp.float32
        params = jax.tree_util.tree_map(
            lambda x: x.astype(_pass_dtype) if eqx.is_inexact_array(x) else x,
            params,
        )

        conditions, _weights, key = _build_conditions(
            problem, key, t0, t1,
...
def train(problem_class, config):
    ...
    Each time slab is trained in two passes: a float32 pass to get cheap
    early progress, then a float64 pass to converge. Steady problems are
    treated as a single slab.
    """
    config.resolve_precision_defaults(double_precision=True)
    ...
    precision_switch_events: list = []  # float32 → float64 transitions
    ...
    results: dict = {
        ...
        "precision_switch_events": precision_switch_events,
    ...
    for is_dry_run, t0, t1 in _slab_seq:
        ...
        # Accumulate lambda_history across both precision passes for this slab
        lambda_history_full: list[float] = []
        ...
        for precision_pass, precision_bits in enumerate([32, 64]):
            ...
            # Cast carried-over parameters to this pass's precision
            _pass_dtype = jnp.float64 if precision_bits == 64 else jnp.float32
            params = jax.tree_util.tree_map(
                lambda x: x.astype(_pass_dtype) if eqx.is_inexact_array(x) else x,
                params,
            ...
            plotter.log(f"\n{'─'*60}")
            plotter.log(f"  Slab (t={t0:.4f}→{t1:.4f}) | Pass {precision_pass + 1}/2 (float{precision_bits})")
            plotter.log(f"{'─'*60}")
            ...
            if precision_pass == 0:
                opt_state      = solver.init_state(opt_key, initial_radius=config.initial_radius)
            target_rho     = config.target_rho
            lambda_history: list[float] = []  # per-pass, for has_passed_minimum detection
            slab_step      = 0
            ...
            while True:
                ...
                if not is_dry_run:
                    ...
                    lambda_history_full.append(float(opt_state.lam))
                ...
                if len(lambda_history) > config.lambda_grace_period and target_rho != 0.5:
                    if has_passed_minimum(lambda_history, window_size=config.lambda_history_size):
                        if precision_pass == 0:
                            pass_exit   = True
                            exit_reason = "reached float32 plateau"
                        else:
                ...
                if precision_pass == 0:
                    if opt_state.radius < 0.1:
                        pass_exit   = True
                        exit_reason = "radius below 0.1"

                if opt_state.radius < 10 * config.min_radius:
                    pass_exit   = True
                    exit_reason = "radius below threshold"
                elif precision_pass == 0 and slab_step >= config.float32_steps:
                    pass_exit   = True
                    exit_reason = f"reached float32 step limit ({config.float32_steps})"

                if pass_exit:
                    # Fresh evaluation at pass exit
                    u_pred = predict_to_current_slab(
                        frozen_slabs, params, static, t0, t1, all_coords, t_col
                    )
                    pure_l2, rel_l2 = compute_l2_error(u_pred, u_ref_flat, t1, t_col, plotter.channels)
                    ...
                    if precision_pass == 0:
                        plotter.mark_precision_switch(global_step)

                        if not is_dry_run:
                            precision_switch_events.append({
                                "global_step": global_step,
                                "wall_time":   t_total,
                                "t0":          t0,
                                "t1":          t1,
                    ...
                    plotter.log(f"  {exit_reason} — exiting pass")
                    l2_scalar = float(np.mean(list(rel_l2.values())))
                    plotter.log(f"  pass {precision_pass + 1}/2 | rel error = {l2_scalar:.3e}")

                    if precision_pass == 1:
                        plotter.wait()
                        plotter.log(f"  radius below threshold — moving to next slab.")
                        plotter.log(
        ...
        if not is_dry_run:
            # Single authoritative evaluation after both passes complete
            ...
            if not is_steady_state:
                ...
                # ── Full slab record (backward-compat, now consistent) ──────
                results["slabs"].append({
                    ...
                    "rel_l2":          rel_l2_slab,
                    "pure_l2":         pure_l2_slab,
                    "lambda_history":  lambda_history_full,   # both passes
                ...
                results["slabs"].append({
                    ...
                    "rel_l2":         rel_l2_slab,
                    "pure_l2":        pure_l2_slab,
                    "lambda_history": lambda_history_full,
                    "final_weights":  list(weights),
                    "n_steps":        slab_step,
                })
